...
public class MainActivity extends Activity {
    ...
    private void displayData() {
        if (relevantPhotos != null) {
            ...
            Call<List<Photo>> call = service.getFeed();
            call.enqueue(new Callback<List<Photo>>(){
                @Override
                public void onResponse(Call<List<Photo>> call, Response<List<Photo>> response){
                    List<Photo> photos = response.body();
                    // the first items not interesting to us, get the last <n>
                    relevantPhotos = new ArrayList<>(photos.subList(photos.size() - PHOTO_COUNT, photos.size()));
                    populateGrid();
                }

                @Override
                public void onFailure (Call<List<Photo>> photos, Throwable t){
